        ...
        <section className='top-0 left-0 w-0 h-0 fixed' id="windows-container">
          ...
          {openWindows.map(({ name, type, content }) => (
            <Draggable handle=".handle"
              ...
              defaultPosition={{ x: window.innerWidth / 2 - 192, y: window.innerHeight / 2 - 192 }}
              scale={1}
              bounds="#limites"
              key={name}
            >
              <div className="absolute z-10 min-w-96 min-h-96 flex flex-wrap flex-col gap-3 p-2 bg-[rgba(0,0,0,0.7)]
            backdrop-blur-xl translate-x-1/2 translate-y-1/2 rounded-md bg-[#070A11] border border-solid  border-blue rounded-br-lg"
                style={{
